...
def test_update_planned_session_marks_missed(db_session: Session) -> None:
    user = create_user(db_session, "athlete@example.com")
    space = create_space(db_session, user)
    session = create_planned_session(
        space.id,
    ...
    response = update_planned_session(
        space.id,
        session.id,
        PlannedSessionUpdateRequest(status="missed", modification_note="Skipped because of travel."),
        user,
        db_session,
    )

    assert response.status == "missed"
    assert response.modification_note == "Skipped because of travel."
